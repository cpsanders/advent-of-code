...
def calculate_middle_sums(rules_path: Path, updates_path: Path) -> None:
    ...
    with updates_path.open("r") as file:
        for i, line in enumerate(file):
            ...
            if update_is_valid:
                valid_sum += page_update[len(page_update) // 2]
            else:
                sorted_sum += page_update[len(page_update) // 2]

    print(f"Originally-correct sum: {valid_sum}")
    print(f"Originally-incorrect sum: {sorted_sum}")
